...
class LLVMBackendVisitor() satisfies Visitor {
    ...
    shared actual void visitCompilationUnit(CompilationUnit c) {
        ...
        for (str->id in strings) {
            ...
            result += "@.str``id``.object = private unnamed_addr constant
                        [3 x i64]
                        [i64 0, i64 ``str.size``, i64 ptrtoint([``str.size`` x i8]*
                        @.str``id``.data to i64)]\n";
            result += "@.str``id`` = alias i64* bitcast([3 x i64]* @.str``id``.object to i64*)\n";
        }
    ...
    shared actual void visitInvocation(Invocation i) {
        ...
        value args = i.arguments.get(llvmData);
        assert(is [Anything*] args);
        value name = m.name.name;
        value spreadable = args.narrow<LLVMExpression|String>().map((x) => ["i64* ", x])
            .fold<[LLVMExpression|String *]>([])((x,y) =>
                x.append(y)).withTrailing(")")
